...
class ClosedMatchSearchQueriesController < ClosedMatchesController
  def show
    ...
    if params[:q] && params[:q].strip.blank?
      ...
      redirect_to closed_matches_path(preserved_params)
      return
    end
    ...
    render template: 'closed_matches/index'
  end

  private

  def filter_params
    # Override to make the Clear button go to the closed_matches index
    super.merge(controller: 'closed_matches', action: 'index')
  end
