...
module Reports
  class HousedAddressesController < ApplicationController
    ...
    def column_transformations
      @column_transformations ||= report_schema.values.map { |descriptor| descriptor[:display_transform] }
    end
    helper_method :column_transformations
    ...
    private def report_schema
      @report_schema ||= {
        ...
        Translation.translate('Did you or this client use external software to help with housing?') => { query: md_b_t[:external_software_used], display_type: :boolean },
