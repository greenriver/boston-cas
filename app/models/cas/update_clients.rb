module Cas
  class UpdateClients
    ...
    def attributes_for_client(project_client)
      ...
      [:vispdat_length_homeless_in_days, :veteran_status, :calculated_chronic_homelessness, :hivaids_status, :chronic_health_condition, :ssn_quality_code, :dob_quality_code, :primary_race, :ethnicity, :gender].each do |key|
    ...
    def project_client_attributes
      @project_client_attributes ||= [:first_name, :last_name, :ssn, :date_of_birth, :veteran_status, :calculated_chronic_homelessness, :ssn_quality_code, :dob_quality_code, :hivaids_status, :chronic_health_condition, :mental_health_problem, :domestic_violence, :physical_disability, :disabling_condition, :calculated_first_homeless_night, :calculated_last_homeless_night, :primary_race, :ethnicity, :gender, :substance_abuse_problem, :developmental_disability, :sync_with_cas, :disability_verified_on, :housing_assistance_network_released_on, :dmh_eligible, :va_eligible, :hues_eligible, :hiv_positive, :housing_release_status, :vispdat_score, :vispdat_length_homeless_in_days, :vispdat_priority_score, :us_citizen, :asylee, :ineligible_immigrant, :lifetime_sex_offender, :meth_production_conviction, :family_member, :child_in_household, :days_homeless, :date_days_homeless_verified, :who_verified_days_homeless, :days_homeless_in_last_three_years, :days_literally_homeless_in_last_three_years, :ha_eligible, :cspech_eligible, :income_total_monthly, :alternate_names, :congregate_housing, :sober_housing, :enrolled_project_ids, :active_cohort_ids, :assessment_score, :ssvf_eligible, :rrh_desired, :youth_rrh_desired, :rrh_assessment_contact_info, :rrh_assessment_collected_at, :enrolled_in_th, :enrolled_in_sh, :enrolled_in_so, :enrolled_in_es, :requires_wheelchair_accessibility, :required_number_of_bedrooms, :required_minimum_occupancy, :requires_elevator_access, :neighborhood_interests, :interested_in_set_asides]
    end
    ...
    def fix_incorrect_available_candidate_clients
      MatchRoutes::Base.all_routes.each do |route|
        ...
        clients.each do |c|
          next unless c.client_opportunity_matches.on_route(route).active.none? && c.client_opportunity_matches.success.none?
