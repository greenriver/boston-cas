        ...
        csv.each do |row|
          ...
          if row['Project, Tenant, or Sponsor based?'] == 'Project-Based'
            ...
            row['# of Units available if project-based'].to_i.times do |i|
              ...
              voucher_count += 1
              units << Unit.create!(name: i + 1, available: true, building: building, elevator_accessible: building.elevator_accessible_default)
            end
          else
          ...
          SubProgram.create(name: row['Subprograms'], program: p, program_type: row['Project, Tenant, or Sponsor based?'], building: building, vouchers: vouchers, service_provider: s, sub_contractor: sc, housing_subsidy_administrator: hsa)
          count += 1
        end
      end
