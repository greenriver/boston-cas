...
class CleanupClientSearchQueriesTask
  ...
  def perform
    with_lock do
      ClientSearchQuery.transaction do
        cleanup_old_queries
      end
    end
  end
  ...
  def cleanup_old_queries
    ...
    ClientSearchQuery.where(updated_at: ..cutoff_date).delete_all
  end

  def with_lock(&block)
    lock_name = self.class.name.demodulize
    ApplicationRecord.with_advisory_lock(lock_name, timeout_seconds: 0, &block)
  end
end
