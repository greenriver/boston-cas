class ConvertShelterAgencyHousingDateStepsToHsaSteps < ActiveRecord::Migration
  def up
    ...
    shelter_agency_future_decisions = MatchDecisions::Base.with_deleted.where(type: 'MatchDecisions::RecordClientHousedDateShelterAgency', status: nil)
  ...
  def down
    hsa_pending_decisions = MatchDecisions::Base.with_deleted.where(type: 'MatchDecisions::RecordClientHousedDateHousingSubsidyAdministrator', status: 'pending')
    hsa_pending_decisions.each do |d|
      d.update(type: 'MatchDecisions::RecordClientHousedDateShelterAgency')
    end
    hsa_future_decisions = MatchDecisions::Base.with_deleted.where(type: 'MatchDecisions::RecordClientHousedDateHousingSubsidyAdministrator', status: nil)
