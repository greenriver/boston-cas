namespace :cas_seeds do
  ...
  task all: [:create_opportunities, :create_rules, :create_services, :ensure_all_users_have_contacts, :create_match_decision_reasons, :create_admin_user]
  ...
  task create_admin_user: [:environment, 'log:info_to_stdout'] do
    ...
    unless /production|staging|test/.match?(Rails.env)
