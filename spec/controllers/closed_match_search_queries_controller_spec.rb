...
RSpec.describe ClosedMatchSearchQueriesController, type: :controller do
  ...
  describe 'GET #show' do
    ...
    it 'redirects to new search query when different search term is provided' do
      ...
      expect(response).to redirect_to(/closed_match_search_queries\/[\w-]+/)
      redirected_id = response.location.match(/closed_match_search_queries\/([\w-]+)/)[1]
    ...
    it 'redirects to main index when empty search is submitted' do
      ...
      expect(response).to redirect_to(closed_matches_path(current_route: 'MatchRoutes::Default'))
    end
  end
end
