...
RSpec.describe ImportedClientSearchQueriesController, type: :controller do
  ...
  before do
    authenticate admin
    admin_role.can_manage_imported_clients = true
    admin_role.save!
  ...
  describe 'GET #show' do
    ...
    it 'renders the imported_clients index template' do
      get :show, params: { id: search_query.id }
      expect(response).to render_template('imported_clients/index')
    end
    ...
    it 'redirects to new search query when different search term is provided' do
      ...
      expect(response).to redirect_to(/imported_client_search_queries\/[\w-]+/)
      redirected_id = response.location.match(/imported_client_search_queries\/([\w-]+)/)[1]
