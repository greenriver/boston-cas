...
RSpec.describe 'Running the match engine...', type: :request do
  ...
  let!(:age_25_clients) { create_list :client, 2, date_of_birth: Date.current - 16.years }
  let!(:age_30_clients) { create_list :client, 1, date_of_birth: Date.current - 30.years }
  ...
  #     expected_ids = Client.where("date_of_birth < ? AND date_of_birth > ?", Date.current - 16.years, Date.current - 25.years).map(&:id)
