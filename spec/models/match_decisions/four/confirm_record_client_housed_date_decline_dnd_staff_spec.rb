...
RSpec.describe MatchDecisions::Four::ConfirmRecordClientHousedDateDeclineDndStaff, type: :model do
  ...
  let(:decision) { match.four_confirm_record_client_housed_date_decline_dnd_staff_decision }
  ...
  describe '#notifications_for_this_step' do
    it 'notifies DND for review plus the standard HSA-decision stakeholders' do
      expect(decision.notifications_for_this_step).to contain_exactly(
        Notifications::Four::ConfirmRecordClientHousedDateDeclineDndStaff,
  ...
  describe 'StatusCallbacks' do
    ...
    describe '#decline_overridden' do
      it 're-initializes the original RecordClientHousedDateHousingSubsidyAdministrator decision' do
        decision.update!(status: 'decline_overridden')
        expect do
          decision.run_status_callback!(user: admin_contact)
        end.to change {
          match.four_record_client_housed_date_housing_subsidy_administrator_decision.reload.status
        }.to('pending')
      end
    end
    ...
    describe '#decline_overridden_returned' do
      it 're-initializes the original decision and uninitializes itself' do
        ...
        aggregate_failures do
          expect(match.four_record_client_housed_date_housing_subsidy_administrator_decision.reload.status).to eq('pending')
