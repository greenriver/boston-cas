...
RSpec.describe MatchDecisions::Four::RecordClientHousedDateHousingSubsidyAdministrator, type: :model do
  ...
  describe 'StatusCallbacks#declined' do
    ...
    it 'does not create a second confirm-decline decision if one already exists' do
      ...
      expect do
        decision.run_status_callback!(user: admin_contact)
      end.not_to(change { MatchDecisions::Four::ConfirmRecordClientHousedDateDeclineDndStaff.where(match: match).count })
