...
RSpec.describe Rules::AgeGreaterThanX, type: :model do
  describe 'clients_that_fit' do
    let!(:rule) { create :age_greater_than_x }

    let!(:bob) { create :client, first_name: 'Bob', date_of_birth: Date.current - 17.years }
    let!(:roy) { create :client, first_name: 'Roy', date_of_birth: Date.current - 18.years }
    let!(:mary) { create :client, first_name: 'Mary', date_of_birth: Date.current - 19.years }
    let!(:sue) { create :client, first_name: 'Sue', date_of_birth: nil }

    let!(:positive) { create :requirement, rule: rule, positive: true, variable: 18 }
    let!(:negative) { create :requirement, rule: rule, positive: false, variable: 18 }
    ...
    context 'when positive' do
      ...
      it 'does not contain Bob (17 y.o.)' do
    ...
    context 'when negative' do
      ...
      it 'does not contain Mary (19 y.o)' do
