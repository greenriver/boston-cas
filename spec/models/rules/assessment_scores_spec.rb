...
RSpec.describe Rules::AssessmentScoreGreaterThanZero, type: :model do

  let!(:rule) { create :assessment_score_greater_than_0 }

  let!(:bob) {
    client = create :client, first_name: 'Bob', assessment_score: 0
  }
  let!(:roy) {
    client = create :client, first_name: 'Roy', assessment_score: 1
  }
  let!(:mary) {
    client = create :client, first_name: 'Mary', assessment_score: 0
  }
