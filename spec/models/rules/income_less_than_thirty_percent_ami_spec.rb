...
RSpec.describe Rules::IncomeLessThanThirtyPercentAmi, type: :model do

  let!(:low_income_rule) { create :income_less_than_30_percent_ami }

  let!(:ami) { Config.get(:ami) }
  let!(:ami_partial) { (ami * 0.3) / 12 }
  ...
  describe 'clients_that_fit IncomeLessThanThirtyPercentAmi' do
