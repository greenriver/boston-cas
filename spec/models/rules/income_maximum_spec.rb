...
RSpec.describe Rules::IncomeMaximum, type: :model do
  describe 'clients_that_fit' do
    let!(:rule) { create :income_maximum }
    ...
    context 'when positive' do
      ...
      it 'contains low income client' do
        expect(clients_that_fit.ids).to include low_income_client.id
      end
      it 'contains nil income client' do
        expect(clients_that_fit.ids).to include nil_income_client.id
      end
      it 'does not contain medium income client' do
        expect(clients_that_fit.ids).to_not include medium_income_client.id
      end
      it 'does not contain high income client' do
        expect(clients_that_fit.ids).to_not include high_income_client.id
      end
    end
    ...
    context 'when negative' do
      ...
      it 'contains medium income client' do
        expect(clients_that_dont_fit.ids).to include medium_income_client.id
      end
      it 'contains high income client' do
        expect(clients_that_dont_fit.ids).to include high_income_client.id
      end
      it 'does not contain low income client' do
        expect(clients_that_dont_fit.ids).to_not include low_income_client.id
      end
      it 'does not contain nil income client' do
        expect(clients_that_dont_fit.ids).to_not include nil_income_client.id
      end
    end
  end
end
