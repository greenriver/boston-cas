...
RSpec.describe "Expressing neighborhood interests", type: :request do
  ...
  context "by an administrator" do
    ...
    it "stores multiple values" do
      post deidentified_clients_path, params: { deidentified_client: { client_assessments_attributes: { 0 => {neighborhood_interests: [ "#{cambridge.id}", "#{beacon_hill.id}" ] } } } }
    ...
    it "converts strings to ints" do
      post deidentified_clients_path, params: { deidentified_client: { client_assessments_attributes: { 0 => { neighborhood_interests: [ "#{cambridge.id}" ] } } } }
      expect(response).to redirect_to deidentified_clients_url
